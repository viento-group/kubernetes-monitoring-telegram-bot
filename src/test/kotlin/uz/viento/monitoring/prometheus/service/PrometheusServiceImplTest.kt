...
internal class PrometheusServiceImplTest : AbstractTest() {
    ...
    fun sendPrometheusAlert() {
        ...
        prometheusServiceImpl.sendPrometheusAlert(PrometheusTestData.PROMETHEUS_DATA, setOf("123", "456"), null)
    ...
    @Test
    fun `sendPrometheusAlert - simple format`() {
        doNothing().whenever(telegramService).sendMessages(any(), any(), any())

        prometheusServiceImpl.sendPrometheusAlert(PrometheusTestData.PROMETHEUS_DATA, setOf("123", "456"), "simple")

        val expectedText = SimplePrometheusDataTextConverter.convert(PrometheusTestData.PROMETHEUS_DATA)
        verify(telegramService, times(1))
            .sendMessages(expectedText, setOf("123", "456"), BotType.PROMETHEUS_ALERT_MANAGER)
    }
}